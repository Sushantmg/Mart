...
export default function ProductsPage() {
    ...
    const { cart, addToCart, removeFromCart, totalItems, totalPrice, clearCart } =
        useCart();
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {filteredProducts.length === 0 ? (
                    ...
                    filteredProducts.map((product) => (
                        <div
                            key={product.id}
                            className="bg-white dark:bg-gray-800 rounded-xl shadow p-4 flex flex-col items-center hover:scale-105 transition-all transform duration-50"
                        >
